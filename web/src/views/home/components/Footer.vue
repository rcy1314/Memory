<template>
  <footer id="footer" class="footer-static">
    <div class="footer-content">
      <div
        class="footer-row"
        :class="{ 'center-only': entries.length > 0 && !isValueNotEmpty(icp) }"
      >
        ...
        <div v-if="isValueNotEmpty(icp)" class="footer-info">
          <p class="footer-icp">
            ICP备案号:
...
const site_name = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.site_name)
    ? settingStore.metaSetting?.site_name
    : import.meta.env.VITE_TITLE
)
const site_desc = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.site_desc)
    ? settingStore.metaSetting?.site_desc
    : import.meta.env.VITE_DESC
)
const primary_color = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.primary_color)
...
const bottom_icon = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.bottom_icon)
    ? settingStore.metaSetting?.bottom_icon
    : import.meta.env.VITE_ICON
)
const bottom_desc = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.bottom_desc)
    ? settingStore.metaSetting?.bottom_desc
    : import.meta.env.VITE_DESC
)
const icp = computed(() =>
  isValueNotEmpty(settingStore.metaSetting?.icp) ? settingStore.metaSetting?.icp : ''
)

createStyle('custom_primary_color', `:root {--moment-theme:${primary_color.value} !important;}`)
</script>
<style>
.footer-row {
  display: flex;
